...
import { ApiId, CreateTripExperienceRequest, Experience, ExperienceSearchFilters, PagedResult } from '@app/core/models/api.models';
...
export class ExperiencesService {
  ...
  uploadPhotos(experienceId: ApiId, files: File[], coverImageIndex: number | null = 0): Observable<UploadedImageDto[]> {
    const formData = new FormData();
    files.forEach((file) => formData.append('files', file));
    if (coverImageIndex !== null) {
      formData.append('coverImageIndex', coverImageIndex.toString());
    }
    return this.api.post<UploadedImageDto[]>(`/TripExperiences/${experienceId}/Photos`, formData);
  }

  deletePhoto(experienceId: ApiId, photoId: ApiId): Observable<boolean> {
    return this.api.delete<boolean>(`/TripExperiences/${experienceId}/Photos/${photoId}`);
  }
}
